...
page_ViewReport = new function () {
    ...
    this.bindData = function (_cur, _ps) {
        ...
        accountName = page_ViewReport.variable.accountName;
        status = -1;
        ...
        reportUpdateType = $("#ddlReportUpdateType").val();
        $.ajax({
            type: "GET",
            url: "/handler/Report.ashx",
            data: {
                reportName: reportName, pic: pic, reportType: reportType, reportUpdateType: reportUpdateType, frequency: frequency, departmentId: departmentId, accountName: accountName, fromDate: fromDate, toDate: toDate, isComplete: isComplete, status: status, page: _cur, pageSize: _ps, t: "GetListReport"
            },
    ...
    this.processData = function (data, _cur) {
        $(this.htmlTag.divList).setTemplateURL("/templates/pages/ViewReportNew.htm?v=2");
    ...
    $("#ddlDepartment").live('change', function () {
        page_ViewReport.getPIC($(this).val());
